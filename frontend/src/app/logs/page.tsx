    ...
    <div className="space-y-6 animate-in fade-in duration-500 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <h2 className="text-3xl font-bold text-slate-800 flex items-center gap-3">
            <Terminal className="w-8 h-8 text-blue-600" /> Auditoria & Monitor de Lotes
          </h2>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={triggering}
            className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 text-white px-5 py-2.5 rounded-xl font-semibold text-sm shadow-sm transition-all"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-center justify-between">
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Status do Worker Redis</p>
            <h3 className="text-xl font-bold text-slate-800 mt-1">
              {stats?.redis_connected ? "Operacional & Ativo" : "Desconectado"}
            </h3>
            <p className="text-xs text-slate-400 mt-0.5 font-mono">{stats?.now}</p>
          </div>
          <div className={`p-3.5 rounded-2xl ${stats?.redis_connected ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"}`}>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-center justify-between">
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Chaves Ativas / Fila</p>
            <h3 className="text-2xl font-bold text-slate-800 mt-1">{stats?.active_keys ?? 0}</h3>
            <p className="text-xs text-slate-400 mt-0.5">Memória e checkpoints alocados</p>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-center justify-between">
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Cron Job de Lembretes</p>
            <h3 className="text-xl font-bold text-slate-800 mt-1">Recorrente (1h em 1h)</h3>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredLogs.map((l) => (
                <tr key={l.id} className="hover:bg-slate-50/70 transition-colors">
                  ...
                  <td className="px-6 py-4 text-xs text-slate-500 font-mono max-w-md truncate">
                    {l.detail || "-"}
                  </td>
                </tr>
              ))}
