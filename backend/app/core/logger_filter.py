...
SECRET_REGEX = re.compile(
    r"(sk-[a-zA-Z0-9_\-]{20,}|Bearer\s+[a-zA-Z0-9_\-\.]+)", re.IGNORECASE
)
...
def _mask_log_value(value):
    ...
    value = CPF_REGEX.sub(
        lambda match: (
            f"***.***.{match.group(0)[-6:-3] if len(match.group(0)) >= 6 else '***'}-**"
        ),
        value,
    )
...
class PIIMaskingFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if isinstance(record.args, tuple):
            ...
            record.args = {
                key: _mask_log_value(value) for key, value in record.args.items()
            }
        return True
