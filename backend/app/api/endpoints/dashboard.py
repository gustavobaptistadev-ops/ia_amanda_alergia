...
async def get_stats(db: AsyncSession = Depends(get_db)):
    ...
    appointments = (
        await db.scalar(
            select(func.count(Appointment.id)).where(Appointment.status == "agendado")
        )
        or 0
    )
    human_contacts = (
        await db.scalar(select(func.count(Contact.id)).where(Contact.bot_active == False))
        or 0
    )
    return {
        "novos_contatos": total_contacts,
        "agendamentos": appointments,
        "em_atendimento": human_contacts,
    }
    ...
    return [
        {
            ...
            "last_message": "",
            "time": "",
            "status": "IA" if c.bot_active else "Humano",
        }
...
async def get_kanban_patients(db: AsyncSession = Depends(get_db)):
    ...
    return [
        {
            "title": "Novo Contato / Triagem",
            "color": "bg-blue-500",
            "patients": columns["triagem"],
        },
        {
            "title": "Agendamento Confirmado",
            "color": "bg-emerald-500",
            "patients": columns["agendado"],
        },
        {
            "title": "Pos-consulta / Feedback",
            "color": "bg-purple-500",
            "patients": columns["pos_consulta"],
        },
        {
            "title": "Retorno de Paciente",
            "color": "bg-amber-500",
            "patients": columns["retorno"],
        },
        {
            "title": "Atendimento Humano",
            "color": "bg-rose-500",
            "patients": columns["atendimento_humano"],
        },
    ]
