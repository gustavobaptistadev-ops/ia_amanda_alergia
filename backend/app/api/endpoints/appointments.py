...
async def list_appointments(
    ...
    query = select(Appointment, Contact.phone_number).join(Contact, Appointment.contact_id == Contact.id)
...
async def create_appointment(data: AppointmentCreate, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(select(Contact).where(Contact.phone_number == data.phone_number))
...
async def update_appointment(appointment_id: str, data: AppointmentUpdate, db: AsyncSession = Depends(get_db)):
