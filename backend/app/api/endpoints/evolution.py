    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            if r.status_code == 200:
                # Evolution v2 returns {"instance": {"instanceName": "...", "state": "open" | "connecting" | "close"}}
                return r.json()
...
async def get_instance_qr():
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            logger.info(f"GET /instance/connect -> {r.status_code}: {r.text[:120]}")
            if r.status_code == 200:
                data = r.json()
                qr_val = (
                    data.get("base64")
            ...
            return {
                "error": "qr_error",
                "message": f"Erro ao buscar QR Code ({r.status_code}).",
            }
        except Exception as e:
            logger.error(f"Erro ao buscar QR: {e}")
            return {
                "error": "server_error",
                "message": "Erro de comunicacao com o servidor WhatsApp.",
            }
...
async def reconnect_instance():
    async with httpx.AsyncClient(timeout=20.0) as client:
        headers = get_headers()
        try:
            r_logout = await client.delete(
                f"{EVOLUTION_API_URL}/instance/logout/{EVOLUTION_INSTANCE_NAME}",
                headers=headers,
            )
            logger.info(
                f"DELETE /instance/logout -> {r_logout.status_code}: {r_logout.text[:80]}"
            )
        except Exception as e:
            logger.warning(f"Aviso no logout (continuando): {e}")
        # Em Evolution v2, basta dar logout. Ao chamar /qr novamente, ele recria a conexao
        return {
            "status": "pending",
            "message": "Sessão limpa. Aguarde 3 segundos e clique em Gerar QR Code.",
        }
...
async def logout_instance():
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            return {
                "status": "error",
                "message": f"Erro ao desconectar ({r.status_code})",
            }
        except Exception:
...
async def fix_evolution_webhook(request: Request):
    ...
    if (
        request.url.port
        and request.url.port not in (80, 443)
        and not request.headers.get("x-forwarded-host")
    ):
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            r = await client.post(
                f"{EVOLUTION_API_URL}/webhook/set/{EVOLUTION_INSTANCE_NAME}",
                headers=get_headers(),
                json=payload,
            )
            ...
            if r.status_code in (200, 201):
                ...
                if r_v1.status_code in (200, 201):
                    return {
                        "status": "ok",
                        "message": "Webhook (v1) corrigido com sucesso!",
                    }

                logger.warning(
                    "Falha ao configurar webhook no provedor: status=%s", r.status_code
                )
                return {
                    "status": "error",
                    "message": "Não foi possível configurar o webhook no provedor.",
                }
        except Exception:
            logger.exception("Erro de comunicação com o provedor ao configurar webhook")
            return {
                "status": "error",
                "message": "Não foi possível comunicar com o provedor.",
            }
