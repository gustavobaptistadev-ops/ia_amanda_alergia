...
async def get_instance_status():
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            r = await client.get(f'{EVOLUTION_API_URL}/instance/connectionState/{EVOLUTION_INSTANCE_NAME}', headers=get_headers())
...
async def get_instance_qr():
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            r = await client.get(f'{EVOLUTION_API_URL}/instance/connect/{EVOLUTION_INSTANCE_NAME}', headers=get_headers())
...
async def reconnect_instance():
    async with httpx.AsyncClient(timeout=20.0) as client:
        headers = get_headers()
        try:
            r_logout = await client.delete(f'{EVOLUTION_API_URL}/instance/logout/{EVOLUTION_INSTANCE_NAME}', headers=headers)
        ...
        return {'status': 'pending', 'message': 'Sessão limpa. Aguarde 3 segundos e clique em Gerar QR Code.'}

@router.delete('/logout')
async def logout_instance():
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            r = await client.delete(f'{EVOLUTION_API_URL}/instance/logout/{EVOLUTION_INSTANCE_NAME}', headers=get_headers())
