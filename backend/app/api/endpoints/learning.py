...
from app.models.learning import LearningStatus, LearningSuggestion
...
async def ingest_clinical_case(request: ClinicalCaseRequest):
    ...
    if not request.raw_case_text or len(request.raw_case_text) < 10:
        raise HTTPException(
            status_code=400, detail="O texto do caso deve ter pelo menos 10 caracteres."
        )
    ...
    prompt_template = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "Você é um assistente médico especialista em triagem de alergia. Sua tarefa é ler um relato clínico ou histórico de mensagens e estruturá-lo em um aprendizado para uma IA recepcionista.\n\n"
                "REGRAS ESTRITAS:\n"
    ...
    try:
        ...
        return {
            "status": "ok",
            "message": "Caso clínico processado, anonimizado e aguardando aprovação no painel.",
        }
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Erro ao processar o caso via LLM: {e!s}"
        )
...
async def list_suggestions():
    async with AsyncSessionLocal() as session:
        stmt = (
            ...
            .where(LearningSuggestion.status == LearningStatus.PENDING)
...
async def approve_suggestion(suggestion_id: str):
    async with AsyncSessionLocal() as session:
        ...
        KNOWLEDGE_DIR = os.path.join(
            os.path.dirname(__file__), "../../../docs/knowledge_base"
        )
        ...
        from app.core.rag import collection_name, db_url, get_embeddings
        ...
        try:
            ...
            async with AsyncSessionLocal() as train_session:
                query = text(
                    "DELETE FROM langchain_pg_embedding WHERE collection_id = (SELECT uuid FROM langchain_pg_collection WHERE name = :c_name)"
                )
            ...
            for filename in os.listdir(KNOWLEDGE_DIR):
                if filename.endswith(".md"):
                    ...
                    with open(file_p, encoding="utf-8") as file_obj:
            ...
            await session.commit()
            raise HTTPException(
                status_code=500, detail=f"Erro ao treinar RAG com o novo aprendizado: {e}"
            )

        return {
            "status": "ok",
            "message": "Sugestão aprovada e Base de Conhecimento RAG atualizada com sucesso!",
        }
