        ...
        if suggestion and suggestion.upper() != "NONE":
            ...
            async with AsyncSessionLocal() as session:
                new_suggestion = LearningSuggestion(
                    ...
                    context=f"ExtraÃ­do automaticamente do chat"
                )
                session.add(new_suggestion)
                await session.commit()
                logger.info(f"Nova sugestÃ£o de aprendizado criada para o contato {contact_id}")
...
async def evaluate_recent_chats():
    async with AsyncSessionLocal() as session:
        ...
        for c in recent_contacts:
            # Verifica se já gerou sugestão de aprendizado recente para evitar duplicação (nas últimas 24h)
