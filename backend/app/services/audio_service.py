...
def decrypt_whatsapp_media(enc_bytes: bytes, media_key_b64: str, media_type: str = "audio") -> bytes:
    """
    ...
    try:
        ...
        logger.info(f"Áudio WhatsApp .enc descriptografado com sucesso! ({len(decrypted_data)} bytes Opus puro).")
...
async def transcribe_audio_from_base64_or_url(audio_bytes: bytes, filename: str = "audio.ogg") -> str:
    ...
    try:
        ...
        logger.info(f"Enviando {len(audio_bytes)} bytes de áudio para o Whisper-1 (filename={filename})...")
