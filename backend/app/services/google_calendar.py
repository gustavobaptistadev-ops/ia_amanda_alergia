...
def get_calendar_service():
    ...
    if not creds:
        logger.warning(
            "Credenciais do Google não encontradas. Integração com Google Calendar operando em modo Simulação."
        )
        return None
...
def check_availability(date_str: str, period: str = "todos") -> str:
    ...
    try:
        ...
        if weekday == 6:
            return "A clínica não realiza atendimentos aos domingos. Por favor, consulte horários para dias úteis (segunda a sexta) ou sábados pela manhã."
        ...
        if period and period.lower() in ["manha", "manhã"]:
            horarios_trabalho = [
                h for h in horarios_trabalho if int(h.split(":")[0]) < 12
            ]
        ...
        for h in horarios_trabalho:
            ...
            slot_start = datetime.datetime.combine(base_date, slot_time).replace(
        ...
        if livres:
            ...
            return f"Horários livres para {date_str}{period_label}: " + ", ".join(
                sugestoes
            )
        else:
...
async def create_event(
    ...
    """
    from app.core.validators import sanitize_text, validate_cpf
    ...
    from app.services.db_service import create_or_get_appointment
    
    appointment_id = ""
    duplicate_appointment = False
    try:
        dt_start = datetime.datetime.strptime(f"{date_str} {time_str}", "%Y-%m-%d %H:%M")
        appointment_id, duplicate_appointment = await create_or_get_appointment(
            phone_number=phone,
    ...
    try:
        ...
        title_param = urllib.parse.quote(
            f"Consulta Alergia - {patient_name.split()[0] if patient_name else 'Clínica Lifeline One'}"
        )
        details_param = urllib.parse.quote(
            f"Consulta Médica na Clínica Lifeline One.\n{CLINIC_ADDRESS}"
        )
    ...
    try:
        ...
        if appointment_id:
            from app.services.db_service import update_appointment_google_event_id
            await update_appointment_google_event_id(appointment_id, event_id)
        ...
        return (
            f"Aviso técnico (INVISÍVEL PRO PACIENTE): Ocorreu um erro interno na API do Google Calendar ({e!s}), mas o agendamento no banco de dados foi criado! "
...
async def cancel_event(phone: str) -> str:
    ...
    async with AsyncSessionLocal() as session:
            ...
            if service:
                ...
                try:
                    await asyncio.to_thread(
                        service.events()
                        .delete(calendarId=CALENDAR_ID, eventId=apt.google_event_id)
                        .execute
                    )
                except Exception as e:
        ...
        time_fmt = (
            apt.appointment_time.strftime("%d/%m às %H:%M")
    ...
    async with AsyncSessionLocal() as session:
        ...
        if service:
            ...
            try:
                ...
                events = events_result.get("items", [])
                dt_new_start = datetime.datetime.strptime(
                ...
                conflito = False
                for event in events:
                    if event.get("id") == apt.google_event_id:
                        continue  # ignora o evento atual
        ...
        if apt.google_event_id and service:
            try:
                event = await asyncio.to_thread(
                    service.events()
                    .get(calendarId=CALENDAR_ID, eventId=apt.google_event_id)
                    .execute
                )
                event["start"] = {
                ...
                event["end"] = {
                    "dateTime": (dt_start + datetime.timedelta(hours=1)).isoformat(),
                    "timeZone": "America/Sao_Paulo",
                }
                await asyncio.to_thread(
                    service.events()
                    .update(
                        calendarId=CALENDAR_ID, eventId=apt.google_event_id, body=event
                    )
                    .execute
                )
            except Exception as e:
...
async def confirm_event(phone: str) -> str:
    ...
    async with AsyncSessionLocal() as session:
        ...
        stmt_apt = (
            select(Appointment)
            .where(Appointment.contact_id == contact.id, Appointment.status == "agendado")
            .order_by(Appointment.created_at.desc())
            .limit(1)
        )
        res_apt = await session.execute(stmt_apt)
